...
import scala.meta.tokens.Token.{CR, Comment, LF, Space, Tab, CRLF, MultiHS, FF, MultiNL, Indentation}
  ...
  override def parse(code: String, @Nullable fileName: String): slang.api.Tree = {
    ...
    try {
      ...
      val allTokens = metaTree.tokens
        ...
        .filter(t => t.pos.start < t.pos.end && t.isNot[Space] && t.isNot[Tab] && t.isNot[CR] && t.isNot[LF] && t.isNot[CRLF] && t.isNot[MultiHS] && t.isNot[FF] && t.isNot[MultiNL] && t.isNot[Indentation])
