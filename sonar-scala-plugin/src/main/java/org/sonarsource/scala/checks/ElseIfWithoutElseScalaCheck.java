...
public class ElseIfWithoutElseScalaCheck implements SlangCheck {
  ...
  public void initialize(InitContext init) {
    init.register(IfTree.class, (ctx, ifTree) -> {
      ...
      if (ifTree.elseBranch() == null && isAssignment(ctx)) {
        ctx.reportIssue(ifTree.textRange(), MESSAGE);
      }
  ...
  private static boolean isAssignment(CheckContext ctx) {
    Tree firstAncestor = ctx.ancestors().getFirst();
    return firstAncestor instanceof VariableDeclarationTree;
  }
